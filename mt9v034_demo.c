...
#define IMG_WIDTH	    752 //src image width
#define IMG_HEITHT 	480 //src image height
...
///////////////////////////////////////////////////////////////////////////////////
//?��?????????????��????
#define DEF_PIX_FMT	V4L2_PIX_FMT_SGRBG10	//?????��??????????????????UYVY
///////////////////////////////////////////////////////////////////////////////////


#if _DISPLAY_
#define DISP_WIDTH		IMG_WIDTH
#define DISP_HEIGHT	IMG_HEITHT
...
struct bitmap_file_header {
	unsigned short cfType; /* bitmap file type, 'BM', 0x4d42 */
	unsigned long cfSize; /* bitmap file total size, in bytes */
	unsigned long cfReserved; /* reserved feild, must be zero */
	unsigned long cfOffBits; /* image bits array offset from the file begining */
}BITMAP_FILE_HEADER;
typedef
struct bitmap_info_header {
	unsigned long ciSize; /* this header size, with different version the size is var */
	unsigned long ciWidth; /* bitmap width, in pixel */
	unsigned long ciHeight; /* bitmap height, in pixel */
	unsigned short ciPlanes; /* planes on the device, must be 1 */
	unsigned short ciBitCount; /* bitmap pixel depth, bit per pixel */
	unsigned long ciComplressed; /* bitmap compressed format, 0 is no compressed */
	unsigned long ciImageSize; /* bitmap bits data size, in byte */
	unsigned long ciXPelsPerMeter; /* device horizontal pixels per meter */
	unsigned long ciYPelsPerMeter; /* device virtical pixels per meter */
	unsigned long ciClrUsed; /* colors count entries in palette, 0 is all 256 colors */
	unsigned long ciClrImportant; /* important colors, 0 all is important */
}BITMAP_INFO_HEADER;
typedef
struct rgb_quad {
...
void rawtoyuv(unsigned char *Src,unsigned char *Dst,unsigned char size)
{
	unsigned int i,j;
...
int build_gray_bmp_file(int fd,void* raw_pixel_buf, int bmpWidth, int bmpHeight)
{
	struct bitmap_file_header bmpfileHead;
	struct bitmap_info_header bmpInfo;
	struct rgb_quad colors[256];
	int ii;
	int size_per_scanline = ((bmpWidth * 8 + 0x1f)>>5)*4;
	bmpfileHead.cfType = 0x4d42; /* 0x4d42, bitmap type word */
	bmpfileHead.cfSize = sizeof(bmpfileHead) + sizeof(bmpInfo) + 256*4 + size_per_scanline *bmpHeight;
	bmpfileHead.cfReserved = 0; /* must be zero */
	bmpfileHead.cfOffBits = sizeof(bmpfileHead) + sizeof(bmpInfo) + 256*4;
	bmpInfo.ciSize = sizeof(bmpInfo);
	bmpInfo.ciWidth = bmpWidth;
	bmpInfo.ciHeight = -bmpHeight;
	bmpInfo.ciPlanes = 1;
	bmpInfo.ciBitCount = 8;
	bmpInfo.ciComplressed = 0;
	bmpInfo.ciImageSize = bmpWidth*bmpHeight;
	bmpInfo.ciXPelsPerMeter = 0;
	bmpInfo.ciYPelsPerMeter = 0;
	bmpInfo.ciClrUsed = 256;
	bmpInfo.ciClrImportant = 0;
	for(ii = 0; ii < 256; ii++)
	{
		colors[ii].rgbBlue = ii;
		colors[ii].rgbGreen = ii;
		colors[ii].rgbRed = ii;
		colors[ii].rgbReserved = 0;
	}
// write to bitmap file
	ii = write(fd, &bmpfileHead, sizeof(bmpfileHead));
	if(ii != sizeof(bmpfileHead))
	{
		printf("%s\n", "write bmpfileHead failed!");
		return -1;
	}
	ii = write(fd,&bmpInfo, sizeof(bmpInfo));
	if(ii != sizeof(bmpInfo))
	{
		printf("%s\n", "write bmpInfo failed!");
		return -2;
	}
	ii = write(fd,&colors[0], sizeof(colors));
	if(ii != sizeof(colors))
	{
		printf("%s\n", "write palette entries failed!");
		return -3;
	}
	ii = write(fd, raw_pixel_buf, size_per_scanline * bmpHeight);
	if(ii != size_per_scanline * bmpHeight)
	{
		printf("%s\n", "write image data failed!");
		return -4;
	}
	return 0;
}
                       ...
                       char *stdname, struct buf_info *capture_buff_info, struct v4l2_format *fmt)
{
	...
	*numbuffers = reqbuf.count;
	...
	for (i = 0; i < (*numbuffers); i++)
	{
		...
		capture_buff_info[i].start = (char *)mmap(NULL,
		                                          buf.length,
...
//	char *Dst = (char *)malloc(752*480*2);
//	if(Dst == NULL)
//	{
//		perror("buffer malloc fail\r\n");
//		return -1;
//	}
	...
	while(1)
	{
		...
		for (h = 0; h < display_fmt.fmt.pix.height; h++) {
			for (w = 0; w < display_fmt.fmt.pix.width * 2; w++) {
				//*tmp = *tmp + 25;
				tmp++;
				*tmp++ = 128;
...
//		int w = 0;
//		char *tmp = cap_ptr;
//		//char *tmp_dis = dis_ptr;
//		for (h = 0; h < display_fmt.fmt.pix.height; h++) {
//			for (w = 0; w < display_fmt.fmt.pix.width ; w++) {
//				*dis_ptr++  = *tmp++;
//				*dis_ptr++ = 128;
//			}
//		}
